...
export default function Recomment({ teacherId, commentId, content, hasChild })
{
        ...
        <div className={classNames("commentRoot")}>
            <div className={classNames("commentRoot comment")}>
                ...
                <div className={classNames("comment detail")}>
                    {hasChild && <div>
                        <AiOutlineCaretDown/>
                        <button onClick={()=>{ setMore((prev)=> !prev) }} className={classNames("detail button")}>더보기</button>
                    </div>}
                    {hasChild && more && reComment.map((value)=>
                        <Recomment teacherId={value.teacherId} commentId={value.commentId} parentCommentId={value.parentCommentId} content={value.content} hasChild={value.hasChild} />
                    )}
                </div>
            </div>
        </div>
