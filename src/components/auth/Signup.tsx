...
export default function Signup()
{
    ...
    const sendEmail = async () => {
        try{
            instance.post(`/user/send-mail?email=${input.email}`);
        }catch(error){
            console.log(error);
        }
    }
    ...
    const checkCode = async () => {
        if(input.email.length !== 20){
            alert("이메일이 잘못 입력되었습니다.");
        }
        else{
            const result = await instance.delete('/user/check-code', {
                data: {
                    email: input.email,
                    code: code,
                }
        ...
        <div className={classNames("signUp")}>
            ...
            <h1 className={classNames("input-div title")}>회원가입</h1>
                <div className={classNames("input-div input")}>
                    ...
                    <p>계정이 이미 있는 경우에는 <Link style={{color:"#0076C0"}} to="/login">로그인</Link>해주세요</p>
                        ...
                        <ul className={classNames("list ul")}>
                            <li className={classNames("ul email-li")}>
                                ...
                                <input 
                                    ...
                                    value={input?.email} 
                                    onChange={(e)=>{ change(e); }} 
                                    readOnly={certification ? true : false}
                                />
                            </li>
                            <li className={classNames("ul certification-li")}>
                                {certification ? <span style={{color: "#0076C0", fontSize: "14px"}}>인증 완료</span> : <span style={{color:"red", fontSize: "14px"}}>인증 미완료</span>}
                    ...
                    <button onClick={signup} disabled={certification ? false : true} className={classNames("input button")}>회원가입</button>
                </div>
            </div>
        </div>
    )
